...
function AddBookReturnModal({ handleSubmit }: AddBookReturnModalProps) {
  const [isValidate, setIsValidate] = useState(false);

  const [books, setBooks] = useState([]) as any;
  const [readers, setReaders] = useState([]) as any;
  const [bookList, setBookList] = useState([bookInit, bookInit, bookInit, bookInit, bookInit]);
  ...
  return (
    <Modal title="Thêm Phiếu trả sách" modalId="addBookReturnTicket" size="modal-lg">
      <form
        ...
        <button
          ...
          onClick={(e) => {
            ...
            bookList.forEach((book: any) => {
              if (book.id)
                ds_sach.push({id: book.id})
            })

            const newBookLoanTicket = {
              id_the_doc_gia: Number(inputTenDocGia),
              ten_doc_gia: readers.find((reader: any) => reader.id ===Number(inputTenDocGia)).ho_ten,
              ngay_tra: inputNgayMuon,
              ds_sach: ds_sach
            };
...
export default AddBookReturnModal;
