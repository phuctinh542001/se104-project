...
function UpdateBookModal({ idBook, handleSubmit }: UpdateBookModalProps) {
  ...
  useEffect(() => {
    if (idBook !== 0)
      axios
        .get("http://localhost:8000/api/sach/" + idBook)
        .then((response: any) => {
          setInputMaSach(response.data.ma_sach);
          setInputTenSach(response.data.ten_sach);
          setInputTheLoai(response.data.ten_the_loai);
          setInputTacGia(response.data.ten_tac_gia);
          setInputNamXB(response.data.nam_xb);
          setInputNXB(response.data.ten_nxb);
          setInputNgayNhap(response.data.ngay_nhap);
          setInputTriGia(response.data.tri_gia);
          setInputSoLuong(response.data.so_luong);
        })
        .catch((error) => {
          console.log(error);
        });
